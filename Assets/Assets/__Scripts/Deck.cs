...
public class Deck : MonoBehaviour {
	...
	public void MakeCards() {
		// card Names will be the name of cards t obuild
		...
		for (int i=0; i<cardNames.Count; i++) {
			...
			//Set the transform.parent of the new card to anchor
			...
			foreach (Decorator deco in decorators) {
				...
				} else {		//if its not a suit, it's a rank deco
					tGO = Instantiate( prefabSprite ) as GameObject;
					tSR = tGO.GetComponent<SpriteRenderer>();
				...
				//Make the deco Sprites redner above the card
				tSR.sortingOrder = 1;
				//Make the decorator SPrite a child of hte Card
				...
				//Set the sfcale to keep decos from being too big
				if (deco.scale != 1) {
			...
			foreach(Decorator pip in card.def.pips) {
				//Instantiate a Sprite GameObject
				tGO = Instantiate( prefabSprite ) as GameObject;
				//Set the paretn to be the card GameObject
				tGO.transform.parent = cgo.transform;
				//Set the psition to that specified in the XML
	...
	public Sprite GetFace(string faceS) {
		foreach (Sprite tS in faceSprites) {
			//If thsi Sprite has the right name...
			if (tS.name == faceS) {
